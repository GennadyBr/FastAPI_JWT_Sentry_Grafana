...
from tests.test_handlers.test_data import user_data_who_promoted
from tests.test_handlers.test_data import user_data_who_revoke


async def test_add_admin_role_to_user_by_superadmin(
    client, create_user_in_database, get_user_from_database
):
    user_data_for_promotion["roles"] = [PortalRole.ROLE_PORTAL_USER]
    user_data_who_promoted["roles"] = [PortalRole.ROLE_PORTAL_SUPERADMIN]
...
async def test_revoke_admin_role_from_user_by_superadmin(
    ...
    user_data_for_revoke["roles"] = [
        PortalRole.ROLE_PORTAL_USER,
        PortalRole.ROLE_PORTAL_ADMIN,
    ]
    user_data_who_revoke["roles"] = [PortalRole.ROLE_PORTAL_SUPERADMIN]
...
async def test_revoke_admin_role_from_user_by_wrong_type_of_user(
    ...
    user_data_who_revoke["roles"] = roles_of_who_revoke
    for user_data in [user_data_for_revoke, user_data_who_revoke]:
        await create_user_in_database(**user_data)
    resp = client.delete(
        f"/user/admin_privilege?user_id={user_data_for_revoke['user_id']}",
